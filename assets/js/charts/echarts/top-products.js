...
const topProductsInit = () => {
  ...
  $.ajax({
    method:"post",
    data:"totalReussiStudyField",
    url:"/admin/statistics/data/"+ $echartBarTopProducts.id,
    success: function(data){
       jsonDataSet = data["dataset"];
       if ($echartBarTopProducts) {
        const data=jsonDataSet;
        const userOptions = utils.getData($echartBarTopProducts, "options");
        const chart = echarts.init($echartBarTopProducts);
