...
class PreRegistrationController extends AbstractController
{
    ...
    public function index(Request $request): Response
    {
        ...
        if ($fieldForm->isSubmitted() && $fieldForm->isValid()) {
            if (in_array($fieldForm['type']->getData(), ['select', 'checkbox', 'radio']) && count($fieldForm['preRegistrationFormFieldOptions']->getData()) < 2) {
                $this->addFlash('error', 'vous devez saisir au moins 2 options.');
    ...
    public function getResultData(User $user): Response
    {
        ...
        foreach ($temp as $response) {
            ...
            }  else if (is_int(json_decode($response->getResponse()))) {
                $exist = false;
                foreach ($response->getPreRegistrationFormField()->getPreRegistrationFormFieldOptions() as $option) {
                    if ($option->getId() == $response->getResponse()) {
                        $exist = true;
                    ...
                    $responses[$response->getPreRegistrationFormField()->getLabel()] = $value;
                }
            } else
                $responses[$response->getPreRegistrationFormField()->getLabel()] = $response->getResponse();
        }
        if (!empty($responses)) {
            $status = 'hasData';
        } else {
    ...
    public function result(): Response
    {
        ...
        foreach ($users as $user){
            if(!empty($user->getResponses()->toArray()))
                $filled[]=$user;
            else
                $notFilled[]=$user;
        }
       // $users=$filled + $notFilled;
        }else{
            $this->addFlash('error', 'Il n\'y a pas de session de recrutement en cours');
            return $this->redirectToRoute('app_admin');
        }
        return $this->render('pre_registration\result.html.twig', ['users' => $users,'registered'=>$registered,'filled'=>count($filled)]);
    }
            ...
            if ($user->getResult() != null) {
                if ($user->getResult()->getPreRegistration() === true) {
                    $email = (new TemplatedEmail())
                        ...
                        ->subject('[Résultat du sélection des formulaires]')
                ...
                elseif( empty($user->getResponses()->toArray())  ) {
                    $email = (new TemplatedEmail())
                        ->from(new Address('rh@optimajuniorentreprise.com', 'JOIN'))
                        ->to(new Address($user->getEmail(), $user->getFName().' '.$user->getLName()))
                        ->subject('[Rappel formulaire Optima JE]')
                        ->htmlTemplate('email/preregistration_form_reminder.html.twig')
                        ->context([
                            'user' => $user
                        ]);
                    $mailer->send($email);
                ...
                $result->setPreRegistrationEmail(true);
                $user->setResult($result);
                $this->em->persist($result);
                $this->em->flush();
