...
class MainController extends AbstractController
{
    ...
    public function index(): Response
    {
        ...
        {
            if($currentUser->getResponses()->get(0) != null){
                $state='preRegClosedSubmitted';
            }
            else
                $state='preRegClosed';
        }

        if($currentUser->getResult() && $currentUser->getResult()->getPreRegistration() && $currentUser->getResult()->isPreRegistrationEmail()){
        ...
        if($state == 'readyForInterview' ){
            ...
            if($recruitementSession->getBookingForInterview()){
              //  if($today > $recruitementSession->getForBookingInterviewsScheduleEnd() && $today < $recruitementSession->getValidateInterviewsScheduleEnd()){
                if($this->getUser()->getResult()->getPreRegistration() === true && $this->getUser()->getResult()->isPreRegistrationEmail() === true && empty($this->getUser()->getDemandes()->toArray()) && ($this->getUser()->getResult()->getInterview() === null || !$this->getUser()->getInterview() || ($this->getUser()->getInterview() && $this->getUser()->getInterview()->getEnd() > $today ))){
        ...
        $stepReached= (!empty($currentUser->getResponses()->toArray())) + (!empty($currentUser->getInterviewCriterionResults()->toArray())) + ($currentUser->getResult()&& $currentUser->getResult()->getTrialPeriod()&& $recruitementSession->getTrialPeriod()&&$currentUser->getResult()->isTrialPeriodEmail())   ;
