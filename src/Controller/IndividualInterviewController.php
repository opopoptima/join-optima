...
class IndividualInterviewController extends AbstractController
{
    ...
    public function createInterview(Request $request): Response
    {
        $today = new DateTime();
        $recruitmentSession = $this->em->getRepository(RecruitmentSession::class)->findOneBy(["current" => true]);
        if (!$recruitmentSession || ($today > $recruitmentSession->getInterviewsEnd() && $recruitmentSession->getBookingForInterview()) || (($today > $recruitmentSession->getValidateInterviewsScheduleEnd() && !$recruitmentSession->getBookingForInterview())))
            $access = false;
        else
            $access = true;
    ...
    if($this->getUser()->getResult() && $this->getUser()->getResult()->getPreRegistration() === true && $this->getUser()->getResult()->isPreRegistrationEmail() === true && empty($this->getUser()->getDemandes()->toArray()) && ($this->getUser()->getResult()->getInterview() === null || !$this->getUser()->getInterview() || ($this->getUser()->getInterview() && $this->getUser()->getInterview()->getEnd() > $today )))
            $access = true;
        else
            $access = false;
    ...
    public function cancelBookingCandidate(Request $request): Response
    {

        $interview=$this->getUser()->getInterview();
        $demande = new Demande();
    ...
    public function bookInterview(Request $request): Response
    {
        ...
        if ($bookedInterview) {
            ...
            if ($bookedInterview->getStart() > $limintinfAlready && $interview->getStart() > $limitinf && !$interview->getCandidate()) {
                ...
                $this->addFlash("success", "Votre réservation a été bien prise en compte");
                $this->em->flush();
                return $this->redirectToRoute('app_candidate');
            } else {
                ...
                if ($bookedInterview->getStart() < $limintinfAlready) {
                    $this->addFlash("error", "Vous ne pouvez pas modifier votre entretien qu'avant 2 jours de sa date.");
                    return $this->redirectToRoute('app_individual_interview_booking');
                }
                if ($interview->getStart() < $limitinf) {
                    $this->addFlash("error", "Vous devez réserver un entretien avant au moins 24 heures.");
                    return $this->redirectToRoute('app_individual_interview_booking');
                }
            }
...
*/
    ...
    public function submitDemande(Request $request, DemandeRepository $demandeRepository): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $this->addFlash(
                'success',
                'Votre demande a été envoyée avec succès à l\'administrateur.'
            );
            return $this->redirectToRoute('app_candidate');
        }
    ...
    public function createSheet(Request $request): Response
    {
        ...
        if ($SheetPartForm->isSubmitted() && $SheetPartForm->isValid()) {
            if (count($SheetPartForm['interviewsEvaluationCriteria']->getData()) < 1) {
                $this->addFlash('error', 'vous devez saisir au moins un critère.');
    ...
    public function EvaluateCandidate(Request $request, User $candidate): Response
    {
        ...
        $sheetParts = $this->em->getRepository(InterviewsEvaluationSheetPart::class)->findBy([],['sheetOrder'=>'ASC']);
        $criteria = [];
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            foreach ($form->getData()['criteria'] as $sheetKey => $sheet) {
                foreach ($sheet as $rank => $criterion) {
                    ...
                    if( $form->getData()[$sheetKey][$criterion->getId()] ||$form->getData()[$sheetKey][$criterion->getId()."_remark"]){
                        $criterionResult = new InterviewCriterionResult();
                        if($form->getData()[$sheetKey][$criterion->getId()])
                            $criterionResult->setResult(
                                $form->getData()[$sheetKey][$criterion->getId()]
                            );
                        if($form->getData()[$sheetKey][$criterion->getId()."_remark"])
                            $criterionResult->setRemark(
                                $form->getData()[$sheetKey][$criterion->getId()."_remark"]
                            );
            ...
            foreach ($allusers as $user) {
                if ($user->getResult() && ((!$recruitmentSession->getTechnicalTest() && !$recruitmentSession->getCollectiveInterview() && $user->getResult()->getPreRegistration()) || (!$recruitmentSession->getTechnicalTest() && $user->getResult()->getCollectiveInterviews()) || $user->getResult()->getTechnicalTestResult())) {
                    ...
                    foreach ($departments as $department){
                        if(in_array($department,$user->getDepartments()->toArray()))
                            $data[$department->getName()]=$data[$department->getName()]+1;
                    }
                }
            }
        } else {
            $this->addFlash('error', 'Il n\'y a pas de session de recrutement en cours');
            ...
            if ($user->getResult() != null) {
                if ($user->getResult()->getInterview() === true) {
                    $email = (new TemplatedEmail())
                        ...
                        ->htmlTemplate('email/interview_selection_accepted.html.twig')
                        ->context([
                            'user' => $user
                        ]);
                    $mailer->send($email);
                    ...
                    $email = (new TemplatedEmail())
                        ->from(new Address('rh@optimajuniorentreprise.com', 'JOIN'))
                        ->to(new Address($user->getEmail(), $user->getFName() . ' ' . $user->getLName()))
                        ->subject('[Résultat sélection]')
                        ->htmlTemplate('email/interview_selection_refused.html.twig')
                        ->context([
                            'user' => $user
                        ]);
                    $mailer->send($email);
    ...
    public function redirectToCalendar(): RedirectResponse
    {
        ...
        if ($currentRecruitmentSession) {
            ...
            } elseif (($currentRecruitmentSession->getBookingForInterview() && $currentRecruitmentSession->getInterviewsEnd() > $now) || (!$currentRecruitmentSession->getBookingForInterview() && $currentRecruitmentSession->getValidateInterviewsScheduleEnd())) {
