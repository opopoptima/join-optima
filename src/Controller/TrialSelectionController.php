    ...
    public function index(): Response
    {
        ...
        if ($recruitmentSession) {
            if (!$recruitmentSession->getTrialPeriod()){
                $this->addFlash('error',"Il n'y a pas de période d'essai dans cette session");
            ...
            if ($user->getResult() != null) {
                if ($user->getResult()->getInterview() === true) {
                    $email = (new TemplatedEmail())
                        ...
                        ->subject('[Résultat Période d\'essai ] ')
                        ->htmlTemplate('email/trial_selection_accepted.html.twig')
                        ->context([
                            'user' => $user
                        ]);
                    $mailer->send($email);
                    ...
                    $email = (new TemplatedEmail())
                        ...
                        ->to(new Address($user->getEmail(), $user->getFName() . ' ' . $user->getLName()))
                        ->subject('[Résultat Période d\'essai ]')
                        ->htmlTemplate('email/trial_selection_refused.html.twig')
                        ->context([
                            'user' => $user
                        ]);
                    $mailer->send($email);
