    ...
    public function buildForm(FormBuilderInterface $builder, array $options): void
    {
        ...
        if(!$user->getResponses()->isEmpty() && $feedback->getPreRegistrationRating() == null){
            $builder
                ...
                ->add('preRegistrationRemark',TextareaType::class,["required"=>false,"label"=>"Vos remarques et suggestions à propos de la présélection sont les bienvenues"]);
        }
        ...
        if ($user->getInterview() != null && $today > $user->getInterview()->getEnd() && $feedback->getIndividualInterviewsRating() == null) {
            $builder
                ...
                ->add('individualInterviewsRemark',TextareaType::class,["required"=>false,"label"=>"Vos remarques et suggestions à propos cd l'entretien sont toujours appréciées"]);
        }
        if ($user->getResult() != null && $user->getResult()->getTrialPeriod() === true && $today > $recruitmentSession->getTrialPeriodSelectionEnd() && $feedback->getTrialPeriodRating() == null) {
            $builder
                ...
                ->add('trialPeriodRemark',TextareaType::class,["required"=>false,"label"=>"N'hésitez pas à nous faire parvenir vos remarques à propos de la période d'essai"]);
        }
